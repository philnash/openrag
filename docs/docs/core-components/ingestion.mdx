...
3. For each connector you configured, register the redirect URI shown in the TUI in your cloud service's OAuth configuration.
...
4. Optional: Under **Others**, set the **Webhook Base URL (`WEBHOOK_BASE_URL`)** to the base address for your OAuth connector endpoints. If set, the OAuth connector webhook URLs are constructed as `WEBHOOK_BASE_URL/connectors/${provider}/webhook`. This option is required to enable automatic ingestion from cloud storage.
...
4. For each connector, you must register an OpenRAG OAuth redirect URI in your cloud service's OAuth configuration.
...
* Make sure you uploaded only [supported file types](#supported-file-types).
