...
test-ci: ## Start infra, run integration + SDK tests, tear down (uses DockerHub images)
	...
	if [ -n "$$TEST_TOKEN" ]; then \
		...
		HTTP_CODE=$$(curl -k -s -w "%{http_code}" -o /tmp/os_diag.txt -H "Authorization: Bearer $$TEST_TOKEN" -H "Content-Type: application/json" https://localhost:9200/documents/_search -d '{"query":{"match_all":{}}}' 2>/dev/null || echo "000"); \
		echo "HTTP $$HTTP_CODE: $$(cat /tmp/os_diag.txt 2>/dev/null | head -c 150 || echo 'No response')"; \
	fi; \
	echo "$(CYAN)=================================$(NC)"; \
	echo ""; \
